...
export default function Randevular(){
  const[data,setData]=useState([]);const[hiz,setHiz]=useState([]);const[per,setPer]=useState([]);
  ...
  return(<div>
    <div className="ph ph-r"><div><h1>Randevular</h1><p>Oluşturun, onaylayın ve yönetin</p></div><button className="btn btn-p" onClick={()=>setModal("y")}>+ Yeni Randevu</button></div>
    {data.length===0?<div className="card empty"><p>Henüz randevu yok</p><button className="btn btn-p" onClick={()=>setModal("y")}>İlk Randevuyu Oluştur</button></div>:
    <div>
    ...
    {modal==="y"&&<div className="mo-ov" onClick={()=>setModal(null)}><div className="mo" onClick={e=>e.stopPropagation()}><h2>Yeni Randevu</h2><form onSubmit={kaydet}>
      ...
      <div className="fr"><div className="fg"><label>Hizmet</label><select required value={form.serviceId} onChange={e=>setForm({...form,serviceId:e.target.value})}><option value="">Seçin</option>{hiz.map(h=><option key={h._id} value={h._id}>{h.name} — {h.price}₺</option>)}</select></div>
      <div className="fg"><label>Personel</label><select required value={form.personnelId} onChange={e=>setForm({...form,personnelId:e.target.value})}><option value="">Seçin</option>{per.map(p=><option key={p._id} value={p._id}>{p.name}</option>)}</select></div></div>
