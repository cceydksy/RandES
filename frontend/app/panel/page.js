...
import { getAppointments, getServices, getPersonnel, getNotifications, getUnconfirmed, updateConfirmation } from "@/lib/api";
const DT = { "onaylandı": "Onaylandı", "iptal": "İptal", "tamamlandı": "Tamamlandı", "beklemede": "Beklemede" };
const DC = { "onaylandı": "s", "iptal": "d", "tamamlandı": "i", "beklemede": "w" };
export default function PanelHome() {
  const [s, setS] = useState({ r: 0, b: 0, h: 0, p: 0 }); const [son, setSon] = useState([]); const [bild, setBild] = useState([]); const [onz, setOnz] = useState([]); const [lod, setLod] = useState(true); const [toast, setToast] = useState(null);
  useEffect(() => { yukle() }, []);
  ...
  async function onayla(id) { const r = await updateConfirmation(id, "onaylandı"); if (r.success) { setToast({ m: "Randevu onaylandı", t: "s" }); setTimeout(() => setToast(null), 3000); yukle() } else { setToast({ m: r.message || "Hata", t: "e" }); setTimeout(() => setToast(null), 3000) } }
  ...
  return (<div>
    ...
    {onz.length > 0 && <div style={{ marginBottom: 20 }}><h3 className="sec-t">Onay Bekleyen Randevular</h3>{onz.map(o => (<div className="alert" key={o._id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}><p><strong>{o.customerName}</strong> — {o.serviceId?.name} · {new Date(o.appointmentTime).toLocaleString("tr-TR", { day: "2-digit", month: "2-digit", year: "numeric", hour: "2-digit", minute: "2-digit" })}</p><button className="btn btn-sm btn-s" onClick={() => onayla(o._id)}>Onayla</button></div>))}</div>}
